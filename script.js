...
// --- CONFIG ---
const DURATION_VILA = 15; 
const DURATION_VILA_mini = 5; 
...
function insertRestText(tl, groupIndex, duration) {
  ...
  // NYTT: Här sätter vi pausen! 1.5 sekunder efter moveUp innan subtitle kommer
  const SUBTITLE_DELAY_AFTER_MOVE = 1.5; 

  const EXIT_DURATION = 0.8; 
  const EXIT_MARGIN = 0.2;     
  const MIN_READ_TIME = 1.0;
  const MAX_STEP = 1.5;
  ...
  // 5. Underrubrik (Svaret)
  // Nu startar vi timern för innehållet med din önskade paus (1.5s)
  ...
  if (sub) {
    tl.fromTo(sub,
      ...
      `${startLabel}+=${contentStartTime}` // Startar efter 1.5s pausen
    );
    // Skjut fram tiden lite så att bilden kommer efter texten (om bild finns)
  ...
  if (message) {
    const messageDelay = contentStartTime + 0.5; 
    ...
    contentStartTime += 3.5; 
  }

  // 8. LIST-ANIMATION
  if (listItems.length > 0) {
    const absoluteExitStartTime = duration - EXIT_DURATION - EXIT_MARGIN;
    const timeWindowForList = absoluteExitStartTime - contentStartTime - MIN_READ_TIME;
    
    let step = 0;
    if (listItems.length > 1) {
      step = Math.min(MAX_STEP, timeWindowForList / (listItems.length - 1));
      if (step < 0.3) step = 0.5; 
    }

    listItems.forEach((item, i) => {
      const myDelay = contentStartTime + (i * step);
...
function addPanelAnimations(tl) {
  panels.forEach((panel, i) => {
    ...
    if (i === 0) {
      ...
      if (i === 1) insertRestText(tl, 1, DURATION_VILA_mini);
...
function addSecondaryPanelsSegment(tl, panelIndex) {
  secondaryPanels.forEach((twoPanel, index) => {
    ...
    // Sekundära vilor
    tl.to(twoPanel, { rotation: 12 * rotationCoef - (12 - index) * rotationCoef - 90, duration: DURATION_VILA_mini, background: 'linear-gradient(90deg,rgba(255, 180, 200, 1) 0%,rgba(255, 89, 226, 1) 6%,rgba(255, 0, 211, 1) 19%,rgba(255, 0, 0, 1) 72%,rgba(0, 0, 0, 1)  100%)', ease: 'linear' }, '>');
