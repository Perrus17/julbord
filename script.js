...
// --- CONFIG ---
const DURATION_VILA = 25; 
const DURATION_VILA_MINI = 22
...
function insertRestText(tl, groupIndex, duration) {
  ...
  const superTitle = group.querySelector('.super-title');
  ...
  const imgContainer = group.querySelector('.artist-image-container');
  const message = group.querySelector('.artist-message');
  ...
  const SUB_ENTER_DURATION = 0.5;
  const MOVE_UP_DELAY = 1.0;
  
  const SUBTITLE_DELAY_AFTER_MOVE = 1.5; 

  // NYTT: Här bestämmer vi att listan ska starta vid exakt 18 sekunder
  const LIST_FIXED_START_TIME = 10;

  const EXIT_DURATION = 0.8; 
  const EXIT_MARGIN = 0.2;     
  const MIN_READ_TIME = 3.0; // Tid att läsa sista punkten
  const MAX_STEP = 4.5;
  const PUSH_DISTANCE = "-=25px"; 
  ...
  // 4. Titel flyttar UPP
  const elementsToMoveUp = [title];
  if (superTitle) elementsToMoveUp.push(superTitle);
  if (imgContainer) elementsToMoveUp.push(imgContainer);

  tl.to(elementsToMoveUp, 
    {
      y: '-=60px', 
      ease: 'sine.out',
      duration: 0.5
  ...
  // 5. Underrubrik
  let contentStartTime = MOVE_UP_DELAY + SUBTITLE_DELAY_AFTER_MOVE;
  ...
  if (sub) {
    tl.fromTo(sub,
      { opacity: 0, y: 20 },
      {
        ...
        y: 0,
        ease: 'sine.out',
        duration: SUB_ENTER_DURATION
      },
      `${startLabel}+=${contentStartTime}`
    );
    contentStartTime += 0.5;
  }

  // 6. BILD-ANIMATION
  if (imgContainer) {
    contentStartTime += 8.0;
     ...
     contentStartTime += 0.8; 
  }

  // 7. ARTIST MESSAGE ANIMATION
  if (message) {
    const messageDelay = contentStartTime + 1.5; 
    
    if (imgContainer) {
      tl.to(imgContainer, 
        { y: "-=40px", scale: 0.9, duration: 0.5, ease: "sine.inOut" }, 
        `${startLabel}+=${messageDelay}`
      );
    }
    ...
    // Vi behöver inte uppdatera contentStartTime här längre eftersom listan har en fast tid
  }

  // 8. LIST-ANIMATION (Nu med fast starttid!)
  if (listItems.length > 0) {
    const absoluteExitStartTime = duration - EXIT_DURATION - EXIT_MARGIN;
    
    // Använd din fasta starttid (18s) för att räkna ut tidsfönstret
    const timeWindowForList = absoluteExitStartTime - LIST_FIXED_START_TIME - MIN_READ_TIME;
    
    let step = 0;
    if (listItems.length > 1) {
      // Om tidsfönstret är litet, anpassa tempot så allt hinns med
      // Math.max(0.5, ...) garanterar att vi inte får negativa eller superkorta tider
      step = Math.min(MAX_STEP, Math.max(0.5, timeWindowForList / (listItems.length - 1)));
    }

    listItems.forEach((item, i) => {
      // VIKTIGT: Här använder vi nu 18s + steget
      const myDelay = LIST_FIXED_START_TIME + (i * step);
      const myAbsTime = `${startLabel}+=${myDelay}`;

      if (i > 0) {
        const previousListItems = Array.from(listItems).slice(0, i);
        let elementsToPush = [title, ...previousListItems];
        if (superTitle) elementsToPush.push(superTitle);
        if (sub) elementsToPush.push(sub);
        if (imgContainer) elementsToPush.push(imgContainer);
        if (message) elementsToPush.push(message);

        tl.to(elementsToPush, 
          { y: PUSH_DISTANCE, duration: 0.5, ease: "sine.inOut" }, 
          myAbsTime 
        );
        tl.to(listItems[i-1], { opacity: 0.5, duration: 0.5 }, myAbsTime);
      }
  ...
  const elementsToExit = [title, sub, superTitle, imgContainer, message, ...listItems].filter(el => el);
...
function addPanelAnimations(tl) {
  panels.forEach((panel, i) => {
    ...
    if (i === 0) {
      ...
      if (i === 1) insertRestText(tl, 4, DURATION_VILA_MINI);
...
function addSecondaryPanelsSegment(tl, panelIndex) {
  secondaryPanels.forEach((twoPanel, index) => {
    ...
    tl.to(twoPanel, { rotation: '+=15', duration: DURATION_VILA_MINI, background: 'linear-gradient(90deg,rgba(255, 180, 200, 1) 0%,rgba(255, 89, 226, 1) 6%,rgba(255, 0, 211, 1) 19%,rgba(255, 0, 0, 1) 72%,rgba(0, 0, 0, 1)  100%)', ease: 'linear' }, '>');
...
// NYTT: Hämta audio-elementet (se till att <audio id="bg-music"...> finns i HTML)
...
function startPresentation() {
  ...
  // NYTT: Starta musiken
  if (bgMusic) {
    bgMusic.volume = 0.5; // Sätt volym (0.0 till 1.0)
    bgMusic.play().catch(error => {
      console.log("Kunde inte spela upp ljud (webbläsaren kanske blockerade det):", error);
    });
  }
